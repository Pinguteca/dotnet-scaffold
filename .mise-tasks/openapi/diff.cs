#!/usr/bin/env dotnet
//MISE description="Show all OpenAPI changes since the committed baseline"
...
{
    Info("No baseline at openapi-baseline.json - skipping diff");
    return 0;
}

Info("Diffing against openapi-baseline.json...");
var result = await Cli.Wrap("oasdiff")
    .WithArguments(["changelog", baseline, current, "--format", "text"])
...
{
    Err($"oasdiff changelog failed (exit code {result.ExitCode})");
